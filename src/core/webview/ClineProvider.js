...
import { enhance } from "../../shared/modes"
			...
			async (message) => {
				switch (message.type) {
					case "webviewDidLaunch":
						...
						this.configManager
							...
							.then(async (listApiConfig) => {
								...
								let currentConfigName = await this.getGlobalState("currentApiConfigName")
					...
					case "openImage":
						...
						break
					case "openFile":
						openFile(message.text, message.values)
						break
					case "openMention":
    ...
    */
	async getState() {
		const [
			...
			this.getGlobalState("customPrompts"),
			this.getGlobalState("enhancementApiConfigId"),
			this.getGlobalState("experimentalDiffStrategy"),
