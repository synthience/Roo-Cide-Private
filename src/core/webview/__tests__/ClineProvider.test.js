...
// Mock delay module
jest.mock("delay", () => {
	const delayFn = (ms) => Promise.resolve()
...
describe("ClineProvider", () => {
	...
	test("preferredLanguage defaults to English for unsupported VSCode language", async () => {
		...
		vscode.env.language = "unsupported-LANG"
		const state = await provider.getState()
	...
	test("handles mode-specific custom instructions updates", async () => {
		...
		mockContext.globalState.get = jest.fn((key) => {
			if (key === "customPrompts") {
				return existingPrompts
			}
			return undefined
	...
	describe("deleteMessage", () => {
		...
		test('handles "Just this message" deletion correctly', async () => {
			...
			vscode.window.showInformationMessage.mockResolvedValue("Just this message")
			// Setup mock messages
			const mockMessages = [
			...
			provider.getTaskWithId = jest.fn().mockResolvedValue({
		...
		test('handles "This and all subsequent messages" deletion correctly', async () => {
			...
			vscode.window.showInformationMessage.mockResolvedValue("This and all subsequent messages")
			// Setup mock messages
			const mockMessages = [
	...
	describe("getSystemPrompt", () => {
		beforeEach(() => {
			...
			mockAddCustomInstructions.mockImplementation((modeInstructions, globalInstructions, cwd) => {
				return Promise.resolve(modeInstructions || globalInstructions || "")
			})
		})
		const getMessageHandler = () => {
			const mockCalls = mockWebviewView.webview.onDidReceiveMessage.mock.calls
		...
		test("uses code mode custom instructions", async () => {
			...
			const mockAddCustomInstructions = jest.requireMock(
				"../../prompts/sections/custom-instructions",
			...
			const promptHandler = mockWebviewView.webview.onDidReceiveMessage.mock.calls[0][0]
		...
		test("uses correct mode-specific instructions when mode is specified", async () => {
			...
			provider.resolveWebviewView(mockWebviewView)
			const architectHandler = mockWebviewView.webview.onDidReceiveMessage.mock.calls[0][0]
