...
function getAdaptiveThreshold(contentLength, baseThreshold) {
...
function evaluateContentUniqueness(searchStr, content) {
...
export function prepareSearchString(changes) {
...
export function evaluateSimilarity(original, modified) {
	return compareTwoStrings(original, modified)
}
// Helper function to validate using diff-match-patch
export function getDMPSimilarity(original, modified) {
...
export function validateEditResult(hunk, result) {
...
function validateContextLines(searchStr, content, confidenceThreshold) {
...
function createOverlappingWindows(content, searchSize, overlapSize = DEFAULT_OVERLAP_SIZE) {
	const windows = []
...
function combineOverlappingMatches(matches, overlapSize = DEFAULT_OVERLAP_SIZE) {
	if (matches.length === 0) {
		return []
	}
...
export function findExactMatch(searchStr, content, startIndex = 0, confidenceThreshold = 0.97) {
	const searchLines = searchStr.split("\n")
...
export function findSimilarityMatch(searchStr, content, startIndex = 0, confidenceThreshold = 0.97) {
	const searchLines = searchStr.split("\n")
	let bestScore = 0
	let bestIndex = -1
...
export function findLevenshteinMatch(searchStr, content, startIndex = 0, confidenceThreshold = 0.97) {
	const searchLines = searchStr.split("\n")
	const candidates = []
	for (let i = startIndex; i < content.length - searchLines.length + 1; i++) {
...
function identifyAnchors(searchStr) {
	const searchLines = searchStr.split("\n")
	let first = null
	let last = null
...
export function findAnchorMatch(searchStr, content, startIndex = 0, confidenceThreshold = 0.97) {
	const searchLines = searchStr.split("\n")
...
export function findBestMatch(searchStr, content, startIndex = 0, confidenceThreshold = 0.97) {
	const strategies = [findExactMatch, findAnchorMatch, findSimilarityMatch, findLevenshteinMatch]
	let bestResult = { index: -1, confidence: 0, strategy: "none" }
...
//# sourceMappingURL=search-strategies.js.map
