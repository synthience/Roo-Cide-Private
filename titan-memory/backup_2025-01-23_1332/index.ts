...
class TitanMemoryServer {
	...
	constructor() {
		...
		// Initialize components and set up handlers
		this.initializationPromise = this.initializeComponents().then(() => {
	...
	private async handleStoreMemory(args: StoreMemoryArgs): Promise<ToolResponse> {
		try {
			console.log("[TitanMemoryServer] Generating embedding for content:", args.content)
			const embedding = await this.nemoApi.generateEmbedding(args.content)

			console.log("[TitanMemoryServer] Storing memory with embedding length:", embedding.length)
			...
			console.log("[TitanMemoryServer] Memory stored successfully:", storedMemory.id)
			return {
				content: [
					{
						type: "text",
			...
			console.error("[TitanMemoryServer.handleStoreMemory] Failed:", error)
	...
	private async handleQueryMemory(args: QueryMemoryArgs): Promise<ToolResponse> {
		try {
			console.log("[TitanMemoryServer] Generating embedding for query:", args.query)
			const queryEmbedding = await this.nemoApi.generateEmbedding(args.query)

			console.log("[TitanMemoryServer] Querying memories with limit:", args.limit || 5)
			const results = await this.memoryManager.queryMemories(queryEmbedding, args.limit || 5)

			console.log("[TitanMemoryServer] Found matches:", results.length)
			return {
				content: [
					{
						type: "text",
			...
			console.error("[TitanMemoryServer.handleQueryMemory] Failed:", error)
			const errorMessage = error instanceof Error ? error.message : "Unknown error occurred"
			return {
				content: [
					{
